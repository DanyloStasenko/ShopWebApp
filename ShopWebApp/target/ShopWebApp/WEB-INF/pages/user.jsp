    ...
    <div id="header">
        <div id="menu">
            <ul>
                ...
                <li class="rightblock"><a class="current" href="<c:url value="/user"/>">Basket</a></li>
            </ul>
        </div>
    </div>
...
<%--<html>
    ...
    <body>
        ...
        <h1>Products List</h1>
            ...
            &lt;%&ndash; Get orders count for current user &ndash;%&gt;
            <c:set var="ordersCount" value="${0}"/>
            <c:forEach items="${orders}" var="order">
                <c:if test="${order.username == pageContext.request.userPrincipal.name}">
                <c:set var="ordersCount" value="${ordersCount+1}" />
            ...
            &lt;%&ndash; If this user has some orders - show his orders.&ndash;%&gt;
            <c:if test="${ordersCount > 0}">
                <h1>Orders List</h1>
                <table class="tg">
                    <tr>
                    ...
                    <c:forEach items="${orders}" var="order">
                        ...
                        <tr>
                            <td>${order.id}</td>
                            <td>${order.productTitle}</td>
                            &lt;%&ndash; Order may be cancelled only if 3 parameters will match: order ID, username, productTitle &ndash;%&gt;
                            <td><a href="<c:url value='/user/cancelorder/${order.id}
                                    /${pageContext.request.userPrincipal.name}
                                    /${order.productTitle}'/>">Cancel</a></td>
                        </tr>
